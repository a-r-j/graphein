...
class graph_format_convert:
    def __init__(self, src_format, dst_format, verbose = 'gnn', columns = None):
        ...
        if (src_format not in supported_format) or (dst_format not in supported_format):
            raise ValueError('Please specify from supported format, ' + '/'.join(supported_format))
        self.src_format = src_format
        self.dst_format = dst_format
        
        supported_verbose_format = ['gnn', 'default', 'all_info']
        if (columns is None) and (verbose not in supported_verbose_format):
            raise ValueError('Please specify the supported verbose mode (' + '/'.join(supported_verbose_format) + ') or specify column names!')
        
        if columns is None:
            if verbose == 'gnn':
                columns = ['edge_index', 'coords', 'dist_mat', 'name', 'node_id']
            elif verbose == 'default':
                columns = ['b_factor', 'chain_id', 'coords', 'dist_mat', 'edge_index', 'kind', 'name', 'node_id', 'residue_name']
            elif verbose == 'all_info':
                columns = ['atom_type', 'b_factor', 'chain_id', 'chain_ids', 'config', 
                            'coords', 'dist_mat', 'edge_index', 'element_symbol', 'kind', 
                            'name', 'node_id', 'node_type', 'pdb_df', 'raw_pdb_df', 'residue_name', 
                            'residue_number', 'rgroup_df', 'sequence_A', 'sequence_B']
        self.columns = columns
        
        self.type2form = {'atom_type': 'str',
                    'b_factor': 'float',
                    'chain_id': 'str',
                    'coords': 'np.array',
                    'dist_mat': 'np.array',
    ...
    def convert_nx_to_dgl(self, G):
        ...
        for i, (_, feat_dict) in enumerate(G.nodes(data=True)):
            for key, value in feat_dict.items():
                if str(key) in self.columns:
                    node_dict[str(key)] = [value] if i == 0 else node_dict[str(key)] + [value]
        
        string_dict = {}
        node_dict_transformed = {}
        for i, j in node_dict.items():
            if i == 'coords':
                node_dict_transformed[i] = torch.Tensor(np.asarray(j)).type("torch.FloatTensor")
            elif i == 'dist_mat':
                node_dict_transformed[i] = torch.Tensor(np.asarray(j[0].values)).type("torch.FloatTensor")
        ...
        for i, (_, _, feat_dict) in enumerate(G.edges(data=True)):
            for key, value in feat_dict.items():
                if str(key) in self.columns:
                    edge_dict[str(key)] = list(value) if i == 0 else edge_dict[str(key)] + list(value)
