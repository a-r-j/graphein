...
from graphein.protein_ligand.edges.distance import compute_distmat
...
def initialise_graph_with_metadata(
    ...
    :type protein_df: pd.DataFrame
    :param ligands_df: Processed Dataframe of ligand structures.
...
def add_nodes_to_graph(
    ...
    :ligands_df: DataFrame of ligand structures containing nodes & initial node metadata to add to the graph.
...
def construct_graphs_mp(
    ...
    config: ProteinLigandGraphConfig = ProteinLigandGraphConfig(),
    num_cores: int = 16,
    ...
    :param config: ProteinLigandGraphConfig to use.
    :type config: graphein.protein.config.ProteinLigandGraphConfig, defaults to default config params
...
def compute_secondary_structure_graph(
    ...
    :raises ProteinLigandGraphConfigurationError: If the protein structure graph is
    ...
    for _, d in g.nodes(data=True):
        if "ss" not in d.keys():
            raise ProteinLigandGraphConfigurationError(
